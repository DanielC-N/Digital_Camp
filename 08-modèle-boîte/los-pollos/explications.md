...
3. Déinissez la marge des éléments `span` à l'intérieur de `nav` sur 10 pixels de haut et en bas, et sur 0 pixel de gauche et à droite.



4. Définissez maintenant quelques règles pour l'élément de classe `content`. Cet élément est un conteneur pour tous les éléments non inclus dans `<nav>`.
...
5. Avec une hauteur de 500 pixels pour `.content`, certains éléments débordent de leur boîte si la fenêtre du navigateur est trop petite.
...
6. Il est maintenant temps de s'attaquer à l'en-tête `.header` et à son élément `h1`: "POULET FRIT".
...
7. Ajoutez maintenant un modèle de boîte à l'élément `.button` nommé "COMMANDEZ MAINTENANT" . À chaque modification, veillez à faire défiler vers le bas si nécessaire pour voir l'effet sur le bouton.
...
| [Précédent](../12-changer-modèles-boîte.md)       |   [Quiz-TODO]()   |  [Suivant : 9-affichage-positionnement](../../09-affichage-positionnement/1-intro-positionnement.md) |
